...
export class LanceDBService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Connect to LanceDB
      console.log('Connecting to LanceDB at:', this.config.uri);
  ...
  async insertEmbeddings(records: LanceDBRecord[]): Promise<void> {
    ...
    try {
      ...
      // Convert to the format expected by vectordb
      const vectordbRecords = records.map(record => ({
        ...record,
        created_at: record.created_at.toISOString(), // Convert Date to string
      })) as Record<string, unknown>[];
      
      await this.table.add(vectordbRecords);
  ...
  async searchSimilar(
    ...
    try {
      ...
      // Convert results back to LanceDBRecord format
      return results.map(result => ({
        ...result,
        created_at: new Date(result.created_at as string), // Convert string back to Date
      })) as LanceDBRecord[];
    } catch (error) {
  ...
  async close(): Promise<void> {
    try {
      if (this.db) {
        ...
        // Note: vectordb Connection doesn't have a close method
        this.db = null;
        this.table = null;
      }
    } catch (error) {
